...
class ProfitChart extends Component {
    ...
    const option = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          return `<b>${axisValue}</b><br/>profit rate: ${(Math.round(value * 10000)/100).toFixed(2)}%`
        }
      },
      grid: { 
        left: '0%',
  ...
  render() {
    ...
    return (
      <div>
      <h2><FormattedMessage id="profit.chart.title"/></h2>
        { earningsHistory.isEmpty() ? 
        (
          <div className={cx('not-available')}>
            <FormattedMessage id="profit.chart.not"/>
          </div>
        ): (
